...
namespace WindBot.Game.AI.Decks
{
   ...
   [Deck("SF", "AI_SF")]
    public class SFExecutor: DefaultExecutor
    {
        public class CardId
        {
            ...
            public const int Xieshendazaihai = 1224927;
            public const int Molichouqu = 59344077;
            public const int Zuzhouzhiguang = 1804528;
            public const int xiaoxuanfeng = 5318639;
            ...
            public const int yinxiongdoushi = 47596607;
            public const int jiliushang = 72283691;

        }
        public SFExecutor(GameAI ai, Duel duel)
            : base(ai, duel)
        {
            ...
            AddExecutor(ExecutorType.Activate, CardId.yinxiongdoushi);
            AddExecutor(ExecutorType.SpellSet, CardId.yinxiongdoushi, SpellSeteffect);
            ...
            AddExecutor(ExecutorType.Summon, CardId.CyberDragon, ming4yun4ying1xiong2chong1chong4ci4ci1ren2_81866673NormalSummon);
            ...
            AddExecutor(ExecutorType.SpellSet, CardId.xiaoxuanfeng);
            AddExecutor(ExecutorType.Activate, CardId.xiaoxuanfeng, DefaultMysticalSpaceTyphoon);
            AddExecutor(ExecutorType.SpellSet, CardId.Xieshendazaihai);
            AddExecutor(ExecutorType.Activate, CardId.Xieshendazaihai);
            AddExecutor(ExecutorType.SpellSet, CardId.Molichouqu);
            AddExecutor(ExecutorType.Activate, CardId.Molichouqu);
            AddExecutor(ExecutorType.SpellSet, CardId.Zuzhouzhiguang);
            AddExecutor(ExecutorType.Activate, CardId.Zuzhouzhiguang);
            ...
            AddExecutor(ExecutorType.Repos, CardId.qianyannajishen, DefaultSFMonsterRepos);
        ...
        public override BattlePhaseAction OnSelectAttackTarget(ClientCard attacker, IList<ClientCard> defenders)
        {
            ...
            if (Bot.HasInMonstersZone(CardId.qianyannajishen) && Bot.GetSpellCount() >=1 )
                return AI.Attack(attacker, attacker);
            return null;
        }
        ...
        {
            IList<int> SelectedCard = new List<int>();
            SelectedCard.Add(CardId.MirrorForce);
            ...
            foreach (ClientCard card in Enemy.GetMonsters())
            {
                if (card.IsCode(CardId.tianshilili) && Bot.LifePoints <= 3400 && Enemy.LifePoints > 2000 && card.IsAttack())
                {
                    return true;
                }
            }
...
private bool si3zhe3su1sheng1_83764718SpellSet()
        ...
        private bool si3zhe3su1sheng1_83764718Activate()
        {
            ...
            if (Bot.HasInHandOrHasInMonstersZone(CardId.Jushenbin) && Bot.GetMonsterCount() >= 1 && Duel.Phase == DuelPhase.Main1 && Bot.Hand.GetCardCount(CardId.Sizhesusheng) >= 2)
            {
                AI.SelectCard(targets);
                return true;
            }
            if (Bot.HasInHand(CardId.XuemoD) && Bot.GetMonsterCount() >= 1 && Duel.Phase == DuelPhase.Main1 && Bot.Hand.GetCardCount(CardId.Sizhesusheng) >= 2)
            {
                AI.SelectCard(targets);
                return true;
            }
